    ...
    <section class="relative max-w-6xl mx-auto px-6 pt-12 sm:pt-20 pb-20">
      ...
      <div class="relative grid lg:grid-cols-[1.05fr_1fr] gap-12 lg:gap-10 items-center">
        ...
        <div class="min-w-0 text-center lg:text-left">
          <p class="anim anim-1 font-mono text-[11px] sm:text-xs tracking-[0.35em] uppercase text-gold/80 mb-5">
            5e &amp; 5.5e&nbsp;·&nbsp;Character Forge
          </p>

          <h1 class="anim anim-2 font-display text-5xl sm:text-6xl xl:text-7xl font-bold text-parchment leading-[0.95] tracking-tight mb-6">
            Forge Your<br />Legend
          </h1>

          <p class="anim anim-3 font-body text-stone-300 text-lg sm:text-xl italic mb-4 max-w-md mx-auto lg:mx-0 leading-relaxed">
          ...
          <div class="anim anim-5 flex flex-col sm:flex-row items-center lg:items-start gap-4 justify-center lg:justify-start">
            <RouterLink
              to="/create"
            ...
            <RouterLink
              to="/saved"
              class="btn-secondary text-sm !py-3 w-full sm:w-auto text-center"
            >
              View Saved
            </RouterLink>
          </div>
        ...
        <div class="anim anim-4 dossier-wrap min-w-0 mx-auto w-full max-w-md lg:max-w-none">
          <article class="dossier card !p-5 sm:!p-6" aria-label="Sample character sheet preview">
            ...
            <header class="flex items-start justify-between gap-3 pb-4 border-b border-stone-700">
              <div class="min-w-0">
                <h2 class="font-display text-xl sm:text-2xl font-bold text-parchment leading-tight truncate">
                  Lyra Dawnbringer
                </h2>
              ...
              <span class="shrink-0 flex flex-col items-center px-2.5 py-1 rounded-lg bg-gold/10 border border-gold/30">
                <span class="text-[9px] uppercase tracking-widest text-gold/70 leading-none">Level</span>
            ...
            <div class="grid grid-cols-6 gap-1.5 sm:gap-2 py-4">
              ...
              >
                ...
                <span
                  class="stat-num text-base sm:text-lg font-bold leading-none mt-1"
                  :class="a.mod >= 0 ? 'text-vivid' : 'text-red-500'"
                >{{ a.mod >= 0 ? '+' + a.mod : a.mod }}</span>
                <span class="stat-num text-[11px] font-semibold text-parchment mt-0.5 leading-none">{{ a.score }}</span>
              </div>
            </div>
            ...
            <div class="grid grid-cols-4 gap-2 py-3 border-y border-stone-700">
              <div v-for="c in combat" :key="c.label" class="text-center">
                <p class="stat-num text-xl sm:text-2xl font-bold text-parchment leading-none">{{ c.value }}</p>
            ...
            <div class="flex flex-wrap gap-1.5 pt-4">
              <span
                v-for="f in features"
                :key="f"
                class="text-xs px-2.5 py-1 rounded-full bg-gold/10 text-gold border border-gold/30"
              >{{ f }}</span>
            </div>
          </article>
        </div>
    ...
    <section class="max-w-5xl mx-auto px-6 pb-28">
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <div
          ...
          :key="feat.title"
          class="feat card border-stone-700 !p-6 group"
          :style="{ animationDelay: 0.5 + i * 0.1 + 's' }"
        >
          <div class="flex items-center justify-between mb-4">
            <div class="feat-icon text-gold" v-html="feat.icon" />
            <span class="font-mono text-[10px] tracking-[0.25em] uppercase text-stone-500">{{ feat.tag }}</span>
          </div>
      ...
      <!-- Also by the author -->
      <div class="also mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 px-5 py-4 border border-stone-700 rounded-xl bg-stone-800/40">
        <div>
          <p class="font-mono text-[10px] text-stone-500 uppercase tracking-[0.25em] mb-1.5">Other tools</p>
...
@keyframes float {
  0%, 100% { transform: translateY(0) rotateX(0) rotateY(-2deg); }
  50%       { transform: translateY(-10px) rotateX(1.5deg) rotateY(1deg); }
}
