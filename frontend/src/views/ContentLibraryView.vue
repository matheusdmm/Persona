<template>
  <div class="max-w-5xl mx-auto px-4 py-8">
    ...
    <!-- Loading -->
    <div v-if="loading" class="flex items-center justify-center py-24 text-stone-500 gap-3" aria-live="polite">
      <LoadingSpinner class="w-5 h-5" />
      <span class="text-sm">Loading {{ activeLabel }}…</span>
    </div>

    <!-- Error -->
    <div v-else-if="error" class="text-center py-24" aria-live="assertive">
  ...
  <TransitionRoot :show="!!selected" as="template">
    <Dialog :open="!!selected" @close="close" class="relative z-50">
      ...
      <!-- Panel positioning -->
      <div class="fixed inset-0 flex flex-col justify-end sm:justify-center sm:items-center sm:p-6">
        <TransitionChild
          as="template"
        ...
        >
          <DialogPanel class="relative bg-stone-900 border border-stone-700 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[88vh] sm:max-h-[82vh] flex flex-col shadow-2xl">
            ...
            <div class="flex items-start justify-between gap-4 px-5 pt-4 pb-3 border-b border-stone-700 shrink-0">
              <div class="min-w-0">
                <DialogTitle class="text-parchment font-semibold text-lg leading-snug">
                  {{ selected?.name }}
                </DialogTitle>
            ...
            <div class="overflow-y-auto px-5 py-5 flex flex-col gap-5">
              ...
              <div v-if="selected && propEntries(selected).length" class="flex flex-col gap-3 pt-1 border-t border-stone-800">
